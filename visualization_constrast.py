...
def visualization(dataset, loader, model, num_samples=100, visul_path='./visualization/SRTransformer'):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            trues_list = [trues[0, idxs[0], idxs[1]] for idxs in bbox_idx]
            if len(bbox_idx) not in [3]:
                continue
            elif len(set(trues_list)) == 1:
                continue
            # if '04183.jpg' != image_name:
            #     continue
            # else:
            #     print(image_name)
            ...
            if not os.path.exists(visul_path):
                os.mkdir(visul_path)
            for pair_id1, idxs in enumerate(bbox_idx):
                for pair_id2 in range(3):
                    if pair_id2 == pair_id1:
                        continue
                    contrast_idxs = bbox_idx[pair_id2]
                    atten_max = torch.max(atten[0, idxs[0], idxs[1]], atten[0, contrast_idxs[0], contrast_idxs[1]])
                    atten_raw = atten_max - atten[0, contrast_idxs[0], contrast_idxs[1]]
                    ...
                    new_img.save(visul_path + '/{}_{}_{}_true_{}_pred_{}_pair_{}_{}.jpg'.format(image_name.split('.')[0], idxs[0], idxs[1], true_label, pred_label, pair_id1, pair_id2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_size', type=int, default=384, help='hidden dimension')
    ...
    parser.add_argument('--enc_layers', type=int, default=0, help='number of encoder layers')
    ...
    # training
    test_visul_path = './visualization/{}_{}_SRTransformer/test_person_3_contrast'.format(args.dataset, args.img_size)
    visualization(test_dataset, test_dataloader, model, num_samples=len(test_dataloader), visul_path=test_visul_path)
    train_visul_path = './visualization/{}_{}_SRTransformer/train_person_3_contrast'.format(args.dataset, args.img_size)
